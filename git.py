...
def result():
	...
	try:
		r = requests.get(url,auth=('chetantarale', '2mm10cs009'))
		print r
		if(r.ok):
			...
			for i in range(1,noOfPages+1):
				#print 'https://api.github.com/repos/Shippable/support/issues?state=open&page='+ str(i)
				#print urlToIssues + str(i)
				pr = requests.get(urlToIssues + str(i),auth=('chetantarale', '2mm10cs009'))
				print pr
